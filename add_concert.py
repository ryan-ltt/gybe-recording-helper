...
def parse_show_page(html: str, url: str) -> dict | None:
    """
    Parse a single setlist.fm show page HTML into the canonical pipeline shape.
    Return None if required fields (date, artist) are missing.
    """
    parser = SetlistParser()
    parser.feed(html)
    if not parser.date or not parser.artist:
        return None

    songs = []
    for _, song_list in parser._sections:
        songs.extend(song_list)

    venue = parser.venue or ''
    if parser.city:
        venue = f'{venue}, {parser.city}' if venue else parser.city

    return {
        'date': parser.date,
        'venue': venue.lower(),
        'songs': [s.lower() for s in songs],
        'setlistfm_url': url,
    }
...
def scrape_setlist(url: str) -> dict:
    ...
    parsed = parse_show_page(html, url)

    if parsed is None or not parsed.get('date'):
        raise ValueError("Could not find date on the page.")

    # Re-parse to get the rich sections (encore labels) needed by build_card.
    # parse_show_page flattens sections; we need the original parser output here.
    inner = SetlistParser()
    inner.feed(html)
    ...
    songs = []
    for i, (section_type, song_list) in enumerate(inner._sections):
    ...
    return {
        "artist": inner.artist,
        "venue": inner.venue or "Unknown Venue",
        "city": inner.city or "",
        "date": parsed['date'],
        "songs": songs,
    }
